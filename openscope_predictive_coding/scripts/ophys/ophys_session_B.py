...
expected_novel_movie_duration = expected_familiar_movie_duration = 300.0
expected_total_duration = 3530.0
session_type = 'B'
...
t0 = tf
file_name = os.path.join(data_path, 'ophys_pilot_randomized_control_%s_1ce104b1011311ac984e647054fd253f.npy' % session_type)
...
# Novel movie block:
t0 = tf
file_name = os.path.join(data_path, 'natural_movie_two_warped_92f1fe36e2c118761cbebcebcc6cd076.npy')
data = np.load(file_name)
number_of_frames = data.shape[0]
runs = 10
frame_length = 2.0/60.0
timing_list = [(ii*frame_length*number_of_frames, (ii+1)*frame_length*number_of_frames) for ii in range(runs)]
curr_stimulus_list, tf = get_block(file_name, timing_list, frame_length, runs, t0=t0, block_key='natural_movie_two')
stimuli.append(curr_stimulus_list)
